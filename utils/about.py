...
def generate_about_tab():
    ...
    The __"Leaderboard"__ tab lists tweets in order of an "Engagement Over Expected (EOE)" metric that assigns each tweet a percentile---100 being the most over-achieving in engagement compared to expectation and 0 being the most under-achieving---based on the predicted and actual number of favorites and retweets. Unfortunately, it's not a simple as

    $$\\text{EOE} = Percent Rank of (Actual Favorites - Predicted Favorites) + (Actual Retweets - Predicted Retweets)$$
    ...
    Nonetheless, despite the above formula being flawed, it should provide a solid heuristic for the rankings generated by the actual methodology. The actual EOE calculation is something like this:

    $EOE = P_i (w_1 * d_1 + w_2 * d_2)$

    where

    $EOE$ = Engagement over Expected

    $P_i$ = $i$'th percentile

    $w_x$ = $\frac{(MAPE_{total} - MAPE_x)}{MAPE_{total}}$

    $MAPE_x$ = mean absolute percent error for $x$

    $x$ = either one of favorite or retweet, where $1$ is favorite and $2$ is retweet

    $d_x$ = actual $x$ - predicted $x$
    ```
    '''
    )
